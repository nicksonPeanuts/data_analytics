...
model_vars <- c("earn", "height", "weight", "male", "education", "mother_education", "father_education", "walk", "exercise", "smokenow", "tense", "angry", "age")

# Rimozione dei NA
dati_model <- na.omit(earnings[ , model_vars])

# Modello completo
model_completo <- lm(earn ~ height + weight + male + education + mother_education + father_education + walk + 
...
model_parsimonioso <- lm(earn ~ education + male + age + height, data = dati_model)
